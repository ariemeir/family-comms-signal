...
|---|---|
| [`server/server.py`](https://github.com/ariemeir/family-comms-signal/blob/main/server/server.py) | The whole bridge in 290 lines. The `on_notify` classifier and the `ble_loop` supervisor are the heart of the system. |
| [`firmware/wifesignal_3ch/wifesignal_3ch.ino`](https://github.com/ariemeir/family-comms-signal/blob/main/firmware/wifesignal_3ch/wifesignal_3ch.ino) | The PNP polarity note and the latch-before-`pinMode` boot-flash fix, with the button semantics documented as a contract with the server. |
| [`app/WifeSignal/SignalViewModel.swift`](https://github.com/ariemeir/family-comms-signal/blob/main/app/WifeSignal/SignalViewModel.swift) | 38 lines: the poll loop and its deliberately forgiving — and knowingly flawed — error policy. |
| [`app/WifeSignal/ContentView.swift`](https://github.com/ariemeir/family-comms-signal/blob/main/app/WifeSignal/ContentView.swift) | The entire UI, including the *Waiting… → Seen ♥* status card. |
| [`device/enclosure/AWifeSignalEnclosure.py`](https://github.com/ariemeir/family-comms-signal/blob/main/device/enclosure/AWifeSignalEnclosure.py) | Parametric CAD as a program: derived dimensions, measured constants, guarded feature steps. |
| [`device/WifeSignalPCB/WifeSignal3ch.kicad_sch`](https://github.com/ariemeir/family-comms-signal/blob/main/device/WifeSignalPCB/WifeSignal3ch.kicad_sch) | The schematic, including the in-drawing notes mapping refs to channels — and the wrong transistor. |
| [`ops/wife-signal`](https://github.com/ariemeir/family-comms-signal/blob/main/ops/wife-signal) | launchd bootstrap, health polling, and Tailscale serve configuration in one operator script. |
| [`server/wifesignal_cli.py`](https://github.com/ariemeir/family-comms-signal/blob/main/server/wifesignal_cli.py) | 69-line BLE REPL for talking to the board directly when the server is stopped. |
